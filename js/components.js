...
function renderSocialIcons(cls = 'footer__social') {
  const icons = SOCIAL_LINKS.map(({ label, href, svg, external }) =>
    `<a href="${href}" class="footer__social-link" aria-label="${label}"${external ? ' target="_blank" rel="noopener"' : ''}>${svg}</a>`
  ).join('');
  return `<div class="${cls}">${icons}</div>`;
}

function phonesHTML() {
  return CONTACT_INFO.phones.map(p => `<span>${p}</span><br>`).join('');
}
...
<aside id="kontak-sidebar" class="ksb" role="dialog" aria-label="Hubungi Kami" aria-modal="true" aria-hidden="true">
  ...
  <div class="ksb__info-col">
    ...
    <div class="ksb__section ksb__section--top">
      ...
      <div class="ksb__phones">${CONTACT_INFO.phones.map(p => `<p>${p}</p>`).join('')}</div>
    </div>
    ...
    <div class="navbar__right">
      <button class="lang-toggle" id="lang-toggle" aria-label="Switch language">
        <span class="lang-toggle__opt" data-lang="en">
          <span class="lang-toggle__opt-inner"><span>EN</span><span aria-hidden="true">EN</span></span>
        </span>
        <span class="lang-toggle__sep" aria-hidden="true">|</span>
        <span class="lang-toggle__opt" data-lang="id">
          <span class="lang-toggle__opt-inner"><span>ID</span><span aria-hidden="true">ID</span></span>
        </span>
      </button>
  ...
  <div class="footer__inner">
    ...
    <div class="footer__cols">
      ...
      <div class="footer__col footer__social-col">
        ...
        ${renderSocialIcons('footer__social')}
      </div>
    </div>
  </div>
</footer>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sideNavSlot) {
    sideNavSlot.outerHTML = renderSideNav(sideNavSlot.dataset.page || '');
  }
  if (footerSlot) {
    footerSlot.outerHTML = footerSlot.dataset.footer === 'v2' ? renderFooterV2() : renderFooter();
  }
  ...
  document.querySelectorAll('.mobile-lang-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      document.getElementById('lang-toggle')?.querySelector(`[data-lang="${btn.dataset.lang}"]`)?.click();
      closeMobileNav();
    });
  });
    ...
    ksbForm.addEventListener('submit', async e => {
      ...
      try {
        const res = await fetch(ksbForm.action, {
          method: 'POST', body: new FormData(ksbForm), headers: { Accept: 'application/json' },
        });
        if (res.ok) {
          ksbForm.reset();
  ...
  // ── Cache nav for subsequent page loads ──────────────────
  try { sessionStorage.setItem('fira-nav-v2', renderNav('__none__')); } catch (_) {}
